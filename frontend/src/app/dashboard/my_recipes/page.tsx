...
export default function RecipesPage() {
  ...
  const fetchIngredients = async (email: string) => {
    try {
      const res = await fetch(`https://altbites.onrender.com/api/ingredients/list?user_email=${encodeURIComponent(email)}`);
  ...
  // Fetch recipes for feed (public + own) 
  const fetchRecipes = async (email?: string) => {
    try {
      ...
      // Fetch rating summary for every recipe 
